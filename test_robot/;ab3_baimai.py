...
def filter_tof(now): 
    yn = now + (0.65 * lst_tof[-1]) 
    return yn 
 
def calculator_adc_(sen_l, sen_r): 
    res = [] 
    volt_l = (sen_l/1023)*3 
    volt_r = (sen_r/1023)*3 
...
def sub_adapter(sub_adaptor): 
    ...
    if lst_adap: 
        ...
        l2, r2 = calculator_adc_(lst_adap[1][2], lst_adap[1][1]) 
        s_l_cm.append(l2) 
        s_r_cm.append(r2) 
...
if __name__ == '__main__': 
    ...
    #ADP 
    ep_sensor_adaptor.sub_adapter(freq=10, callback=sub_adapter) 
    #chasis 
    ep_chasis.sub_position(freq=10, callback=sub_position_handler) 
    ...
    while True: 
            ...
            if counter and abs(targetx - axis_x[-1]) <= 0.1 and abs(targety - axis_y[-1]) <= 0.1: 
                ...
                if s_r_after[-1] > 2 and s_r_after[-1] < 5: 
                    ep_chasis.drive_wheels(w1=speed, w2=-speed, w3=speed, w4=-speed) 
                if s_l_after[-1] > 2 and s_l_after[-1] < 5: 
                    ep_chasis.drive_wheels(w1=-speed, w2=speed, w3=-speed, w4=speed) 
                ep_chasis.drive_wheels(w1=speed, w2=speed, w3=speed, w4=speed) 
    ...
    plt.subplot(3, 2, 1) 
    plt.plot(time_all, axis_y, label='Y', marker=',') 
    plt.ylabel('y Position') 
    plt.xlabel('Time') 
    plt.legend() 
 
    plt.subplot(3, 2, 2) 
    plt.plot(time_all, axis_x, label='X', marker=',') 
    plt.xlabel('Time') 
    plt.ylabel('X Position') 
    plt.legend() 
 
    plt.subplot(3, 2, 3) 
    plt.plot(time_tof, [tof_fill[i] for i in range(len(time_tof))], label='tof_filled', marker=',') 
    plt.plot(time_tof, [lst_tof[i] for i in range(len(time_tof))], label='tof', marker=',') 
    plt.xlabel('Time') 
    plt.ylabel('TOF sensor') 
    plt.legend() 
 
    plt.subplot(3, 2, 4) 
    plt.plot([t_sharp[i] for i in range(len(s_l_after))], s_l_after, label='sharp_left', marker=',') 
    plt.plot([t_sharp[i] for i in range(len(s_l_cm))], s_l_cm, label='sharp_left_filled', marker=',') 
    plt.xlabel('Time') 
    plt.ylabel('Sharp sensor Left') 
    plt.legend() 
 
    plt.subplot(3, 2, 5) 
    plt.plot([t_sharp[i] for i in range(len(s_r_after))], s_r_after, label='sharp_right', marker=',') 
    plt.plot([t_sharp[i] for i in range(len(s_r_cm))], s_r_cm, label='sharp_right_filled', marker=',') 
    plt.xlabel('Time') 
