...
def check_right_clear():
    """หมุนไปทางขวาเพื่อตรวจสอบว่าทางขวาว่างหรือไม่"""
    ep_gimbal.move(pitch=0, yaw=RIGHT_CHECK_ANGLE).wait_for_completed()
    time.sleep(0.1)  # หน่วงเวลาเล็กน้อยเพื่อให้การหมุนเสร็จสมบูรณ์
    is_clear = status_tof and (tof_distance > FRONT_DISTANCE_THRESHOLD)
    ep_gimbal.recenter().wait_for_completed()  # หมุนกลับมาที่ตำแหน่งเดิม
    return is_clear
    ...
    try:
        while True:
            ...
            if status_ss_1 and status_tof:
                # เดินตามกำแพงทางซ้าย
                ep_chassis.move(x=0.2, y=y_speed, z=0, xy_speed=MAX_SPEED).wait_for_completed()

            elif status_ss_1 and not status_tof:
                if check_left_clear():
                    # ถ้าทางซ้ายว่าง ให้เลี้ยวซ้ายแล้วเดินตามกำแพงทางซ้าย
                ...
                elif check_right_clear():
                    # ถ้าทางขวาว่าง ให้เลี้ยวขวาแล้วเดินตามกำแพงทางซ้าย
                    ep_chassis.move(x=0, y=0, z=-90, xy_speed=MAX_SPEED).wait_for_completed()
                    ep_chassis.move(x=0.2, y=0, z=0, xy_speed=MAX_SPEED).wait_for_completed()
                else:
                    # ถ้าทางซ้ายและขวาเป็นทางตัน ให้ถอยหลัง
                    ep_chassis.move(x=-0.2, y=0, z=0, xy_speed=MAX_SPEED).wait_for_completed()
