...
def save_data_to_csv():
    # สร้าง DataFrame สำหรับข้อมูล TOF
    tof_df = pd.DataFrame({'TOF Distance (mm)': tof_data})
    tof_df.to_csv('tof_data.csv', index=False)
    
    # สร้าง DataFrame สำหรับข้อมูล Sharp Left และ Right
    sharp_df = pd.DataFrame({
        'Sharp Left Distance (cm)': sharp_left_data,
        'Sharp Right Distance (cm)': sharp_right_data
    })
