...
def tof_data_handler(sub_info):
    ...
    filtered_r, filtered_l, filtered_tof, filtered_chas_x, filtered_chas_y = filter_signal(adc_r_new, adc_l_new, tof_distance, chasis_data[0], chasis_data[1])

    sharp_right_data.append(filtered_r)
    sharp_left_data.append(filtered_l)
    tof_data.append(filtered_tof)
    chasis_data[0] = filtered_chas_x
    chasis_data[1] = filtered_chas_y
...
def filter_signal(r,l,tof,chas_x,chas_y):
    all_sen = [adc_r_new,adc_l_new,tof_distance,chassis[0],chasis_data[1]]
    sen = [r,l,tof,chas_x,chas_y]
    re = []
    for i in range(len(sen)):
        yn = sen[i]+(0.65*all_sen[i][-1])
        re.append(yn)
    return re[0],re[1],re[2],re[3,re[4]]
